...
from consciousness_bridge import ircg_minimality, ircg_pairwise_minimality
...
def test_all_pairwise_component_projections_are_incomplete_on_union_domain():
    collisions = ircg_pairwise_minimality.pairwise_component_collisions(_audit_domain())
    ...
    assert ircg_pairwise_minimality.every_pairwise_projection_is_incomplete(
        _audit_domain()
    )
...
def test_full_three_component_fingerprint_is_complete_on_itself():
    full = _audit_domain()
    projected = {
        state: ircg_pairwise_minimality.component_projection(
            fingerprint,
...
def test_component_projection_validates_requested_components():
    ...
    with pytest.raises(ValueError, match="At least one component"):
        ircg_pairwise_minimality.component_projection(fingerprint, ())
    with pytest.raises(ValueError, match="must be unique"):
        ircg_pairwise_minimality.component_projection(
            fingerprint,
            ("geometry", "geometry"),
        )
    with pytest.raises(ValueError, match="Unknown IRCG component"):
        ircg_pairwise_minimality.component_projection(
            fingerprint,
            ("geometry", "unknown"),
        )
