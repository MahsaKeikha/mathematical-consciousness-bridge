...
def test_p67_layered_publication_route() -> None:
    root = Path(__file__).resolve().parents[1]
    proof = root / "docs" / "proposition_67_global_integer_optimality_certificate.md"
    figure = root / "docs" / "figures" / "p67_global_integer_optimality_certificate.svg"
    source = root / "src" / "consciousness_bridge" / "global_integer_optimality_certificate.py"
    algorithm_test = root / "tests" / "test_global_integer_optimality_certificate.py"
