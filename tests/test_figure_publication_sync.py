...
P91_FIGURE = "p91_mixed_prevalence_rank_two_flattening_separation.svg"
RAW_PREFIX = (
...
def test_figure_manifest_is_complete_and_byte_exact() -> None:
    ...
    assert manifest["current_frontier"] == "P91"
    assert manifest["current_frontier_figure"].endswith(P91_FIGURE)
...
def test_github_figure_gateway_tracks_p91() -> None:
    gateway = GATEWAY.read_text(encoding="utf-8")
    current = CURRENT.read_text(encoding="utf-8")

    assert CURRENT_SVG.read_bytes() == (DOC_FIGURES / P91_FIGURE).read_bytes()
    assert "## Current theorem frontier: P91" in gateway
    assert P91_FIGURE in gateway
    assert "manifest.json" in gateway

    assert current.startswith("# Current visual frontier: P71-P91")
    assert "## Current theorem frontier: P91" in current
    assert "empirical selected determinant = 1/512" in current
    assert "1/42 < d_inf(P_emp, M_75) <= 1/24" in current
    assert "| P89 |" in current
    assert "| P90 |" in current
    assert "| P91 |" in current


def test_visual_atlas_leads_with_p91_before_historical_frontiers() -> None:
    ...
    assert p91 < p90 < p89
    current = text[p91:p90]
    assert "Current theorem frontier · P91" in current
    assert P91_FIGURE in current
    assert "1/42" in current
    assert "1/24" in current
    assert "proposition_91_mixed_prevalence_rank_two_flattening_separation.md" in current
...
def test_homepage_balances_three_research_stages_and_keeps_history_specialist() -> None:
    ...
    assert research_i < p91 < research_iii < reader_paths
    current = text[p91:research_iii]
    assert "Current theorem frontier · P91" in current
    assert P91_FIGURE in current
    assert "1/42" in current
    assert "1/24" in current
    assert "mixed_prevalence_rank_two_flattening_separation.py" in current
    assert "test_mixed_prevalence_rank_two_flattening_separation.py" in current
    assert "physics_pipeline.svg" in text[research_i:p91]
    ...
    assert "The 91 results form several dependency branches." in text
    assert "all 91 propositions" in text
...
def test_pages_build_bundles_exact_commit_p91_figure(tmp_path: Path) -> None:
    site = tmp_path / "site"
    subprocess.run(
        [
            sys.executable,
    ...
    deployed_p91 = site / "figures" / P91_FIGURE
    assert deployed_p91.read_bytes() == (DOC_FIGURES / P91_FIGURE).read_bytes()

    atlas = (site / "visual-atlas.html").read_text(encoding="utf-8")
    assert f'src="figures/{P91_FIGURE}"' in atlas
    assert f'src="{RAW_PREFIX}' not in atlas

    home = (site / "index.html").read_text(encoding="utf-8")
    assert f'src="figures/{P91_FIGURE}"' in home
    assert f'src="{RAW_PREFIX}' not in home
    research_i = home.index('id="research-i-overview"')
    p91 = home.index('id="p91-frontier"')
    research_iii = home.index('id="research-iii-overview"')
    reader_paths = home.index('id="reader-paths"')
    assert research_i < p91 < research_iii < reader_paths < home.index('id="plain-language"')
    for historical_id in (
        'id="p88-frontier"',
        'id="p87-frontier"',
        'id="p86-frontier"',
