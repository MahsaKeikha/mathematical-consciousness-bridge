...
def test_research_map_starts_with_orientation_before_stage_details():
    ...
    hero = text.index("Eighty-five results, one dependency-aware scientific program")
...
def test_research_map_gives_direct_audit_paths():
    ...
    required = [
        ...
        'index.html#p85-frontier',
    ]
    for token in required:
        assert token in text, token


def test_research_map_presents_p77_through_p85_with_p84_history():
    text = MAP.read_text(encoding="utf-8")
    assert "through Proposition 85" in text
    ...
    p85_navigation = text.index(
        'href="index.html#p85-frontier">Continue to the current P85 frontier</a>',
        frontier,
    )
    p84 = text.index('id="p84"', p83)
    assert p77 < p78 < p79 < p80 < p81 < p82 < p83 < p85_navigation < p84

    assert "P77-P85: from full-law rejection to exact dependency-aware certification" in text
