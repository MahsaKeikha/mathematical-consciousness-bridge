...
from consciousness_bridge.expanded_bounded_primitive_quad_projection_parity_functional_separation import (
    empirical_expanded_primitive_parity_quad_exact,
    expanded_primitive_parity_quad_centered_coefficient_norm_exact,
    p75_box_expanded_bounded_primitive_quad_parity_witness_exact,
    p75_box_p88_linf_lower_bound_exact,
    p75_expanded_primitive_parity_quad_interval_exact,
...
def test_p88_strict_functional_has_exact_value_interval_and_norm():
    ...
    assert empirical_expanded_primitive_parity_quad_exact(empirical, terms) == Fraction(
        -11, 8
    )
    assert p75_expanded_primitive_parity_quad_interval_exact(box, terms) == (
        Fraction(-1),
        Fraction(2),
    )
    assert expanded_primitive_parity_quad_centered_coefficient_norm_exact(terms) == (
        Fraction(24),
        Fraction(-1),
    )
...
def test_p88_is_strictly_stronger_than_complete_p87_on_same_box():
    empirical = _strict_empirical_law()
    box = _strict_box()
...
def test_p88_rejects_nonprimitive_all_even_coefficients():
    terms = (
        ((0, 1), 2),
        ((0, 2), -2),
        ((1, 3), 2),
    ...
    with pytest.raises(ValueError, match="primitive"):
        p75_expanded_primitive_parity_quad_interval_exact(_strict_box(), terms)


def test_p88_rejects_coefficients_outside_declared_box():
    terms = (
        ((0, 1), 1),
        ((0, 2), -1),
        ((1, 3), 4),
    ...
    with pytest.raises(ValueError, match=r"0 < \|c_i\| <= 3"):
        p75_expanded_primitive_parity_quad_interval_exact(_strict_box(), terms)
...
def test_p88_source_keeps_scientific_interpretation_boundary():
    source = (
        __import__(
            "consciousness_bridge.expanded_bounded_primitive_quad_projection_parity_functional_separation",
            fromlist=["dummy"],
        ).__doc__
        or ""
    ).lower()
