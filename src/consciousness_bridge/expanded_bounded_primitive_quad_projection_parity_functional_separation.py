"""P88 exact expanded bounded primitive four-event parity-functional certificates.

P87 closes the complete primitive nonzero integer coefficient box ``0 < |c_i| <= 2``
for four distinct canonical even-parity observables. P88 asks the next finite and
separately testable question at the same four-event order: does enlarging the
complete primitive coefficient box to ``0 < |c_i| <= 3`` reveal shared-parameter
incompatibility that the complete P87 certificate still underestimates?

The P88 family contains 632 sign-normalized primitive coefficient patterns per
four-event subset and 208,560 exact functionals in total. Exact P75 parameter-box
ranges follow from the same multi-affine endpoint argument used by P87, while the
transfer to full-law L-infinity distance uses mass-conservation centering.
...
ExpandedPrimitiveQuadTerm = tuple[tuple[int, ...], int]
ExpandedPrimitiveQuadFunctional = tuple[
    ExpandedPrimitiveQuadTerm,
    ExpandedPrimitiveQuadTerm,
    ExpandedPrimitiveQuadTerm,
    ExpandedPrimitiveQuadTerm,
]
...
class P88PrimitiveQuadParityWitness:
    """Strongest exact witness in the expanded bounded primitive P88 family."""

    lower_bound: Fraction
    terms: ExpandedPrimitiveQuadFunctional
...
def _validate_functional(terms: ExpandedPrimitiveQuadFunctional) -> None:
    if len(terms) != 4:
...
def p88_standard_primitive_weight_pattern_count() -> int:
    """Return the 632 sign-normalized primitive coefficient patterns."""

    return len(_STANDARD_PRIMITIVE_WEIGHT_PATTERNS)


def p88_standard_primitive_quad_count() -> int:
    """Return the 208,560 expanded bounded primitive four-event functionals."""

    return _STANDARD_PRIMITIVE_QUAD_COUNT


def p75_expanded_primitive_parity_quad_interval_exact(
    box: P78ParameterBox,
    terms: ExpandedPrimitiveQuadFunctional,
) -> tuple[Fraction, Fraction]:
    """Return the exact P75 box interval of one expanded primitive functional."""
...
def empirical_expanded_primitive_parity_quad_exact(
    empirical_law: tuple[Fraction, ...],
    terms: ExpandedPrimitiveQuadFunctional,
) -> Fraction:
    """Return the exact empirical value of one expanded primitive functional."""
...
def expanded_primitive_parity_quad_centered_coefficient_norm_exact(
    terms: ExpandedPrimitiveQuadFunctional,
) -> tuple[Fraction, Fraction]:
...
def p75_box_expanded_bounded_primitive_quad_parity_witness_exact(
    empirical_law: tuple[Fraction, ...],
    box: P78ParameterBox,
    ...
    for view_quad in combinations(_STANDARD_VIEW_SETS, 4):
        ...
        for coefficients in _STANDARD_PRIMITIVE_WEIGHT_PATTERNS:
            ...
            if centered_norm <= 0:
                raise RuntimeError("expanded primitive functional unexpectedly has zero norm")
            lower_bound = interval_gap / centered_norm
            terms: ExpandedPrimitiveQuadFunctional = tuple(
                (views, coefficient)
    ...
    if best is None:
        raise RuntimeError("expanded bounded primitive four-event P88 family unexpectedly empty")
    return best


def p75_box_expanded_bounded_primitive_quad_parity_linf_lower_bound_exact(
    empirical_law: tuple[Fraction, ...],
    box: P78ParameterBox,
) -> Fraction:
    """Return the strongest exact lower bound from the 208,560-function P88 family."""

    return p75_box_expanded_bounded_primitive_quad_parity_witness_exact(
        empirical_law,
        box,
    ).lower_bound
...
def p75_box_p88_linf_lower_bound_exact(
    ...
    """Return max(P87, expanded bounded primitive four-event P88 lower bound)."""

    p87 = p75_box_p87_linf_lower_bound_exact(empirical_law, box)
    expanded_primitive_quad = (
        p75_box_expanded_bounded_primitive_quad_parity_linf_lower_bound_exact(
            empirical_law,
            box,
        )
    )
    return max(p87, expanded_primitive_quad)
