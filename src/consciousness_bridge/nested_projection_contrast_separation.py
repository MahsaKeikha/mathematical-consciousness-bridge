"""P82 nested projection-contrast certificates for continuous P75 separation.
...
retains P81 and adds a finite family of dependency-aware residual events formed
from nested cylinders.
...
two additional views. If the P81 parameter-box intervals are

    q(A) in [l_A, u_A],
    q(B) in [l_B, u_B],

then every model law in the box satisfies

    q(A \\ B) in [max(0, l_A-u_B), min(1, u_A-l_B)].

This enclosure is conservative because the extrema of the parent and child
need not occur at the same parameter point. It is nevertheless rigorous and is
...
Therefore empirical separation of a nested residual event from its certified
model-box enclosure, divided by the residual support size, is a valid lower
bound on full-law L-infinity distance.
...
The final P82 box certificate is
    ...
    max(P81 lower bound, P82 nested-contrast lower bound).

It is therefore never weaker than P81 and can be strictly stronger.
...
class P82ContrastWitness:
    """Strongest nested-cylinder residual witness on one parameter box."""
...
def p75_nested_projection_contrast_enclosure_exact(
    ...
    """Return a certified exact-rational enclosure for ``P(A \\ B)``.

    ``B`` must be a strict child cylinder of ``A``. P81 provides exact
    coordinate intervals for ``P(A)`` and ``P(B)`` separately. Interval
    subtraction gives a conservative enclosure for the residual probability.
    Clipping the lower endpoint at zero and the upper endpoint at one uses the
    fact that the residual is itself an event probability.
    """
    ...
    lower = max(Fraction(0), parent_lower - child_upper)
    upper = min(Fraction(1), parent_upper - child_lower)
    if lower > upper:
        raise RuntimeError("nested contrast enclosure became empty")
    return lower, upper
...
def p75_box_nested_projection_contrast_witness_exact(
    ...
    """Return the strongest standard P82 nested-contrast witness on ``box``."""
    ...
    for parent, child in _STANDARD_NESTED_PAIRS:
        ...
        interval_lower = max(Fraction(0), parent_lower - child_upper)
        interval_upper = min(Fraction(1), parent_upper - child_lower)
...
def p75_box_nested_projection_contrast_linf_lower_bound_exact(
    ...
    """Return the strongest standard P82 nested-contrast lower bound."""
...
def p75_box_p82_linf_lower_bound_exact(
    ...
    """Return the P82 box certificate ``max(P81, nested contrast)``."""
...
def certified_p75_linf_branch_and_bound_contrast(
    ...
    *,
    ...
    """Return a global P75 distance bracket using P82 box lower bounds.
