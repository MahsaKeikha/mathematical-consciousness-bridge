...
# Keep the Overview reader-path card synchronized with the current P89 frontier.
index_start = text.find("def promote_index")
index_end = text.find("def promote_visual_atlas", index_start)
if index_start < 0 or index_end < 0:
    raise RuntimeError("P89 Overview promoter was not found")
index_source = text[index_start:index_end]
index_pairs = (
    '("current P88 frontier", "current P89 frontier"),',
    '("sources.html#p88-source", "sources.html#p89-source"),',
    '("P88 sources", "P89 sources"),',
)
index_marker = '("P88 certification ladder", "P89 certification ladder"),'
for pair in index_pairs:
    if pair not in index_source:
        marker_at = text.find(index_marker, index_start, index_end)
        if marker_at < 0:
            raise RuntimeError("P89 Overview replacement marker was not found")
        insertion_at = marker_at + len(index_marker)
        text = text[:insertion_at] + "\n            " + pair + text[insertion_at:]
        index_end += len("\n            " + pair)
        index_source = text[index_start:index_end]
...
print("[repair] finalized P89 Overview, Research Map, links, and Sources promotion")
