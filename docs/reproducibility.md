...
```bash
python -m pytest -q
python -m pytest tests/test_heldout_selected_parity_functional_certification.py
...
## 12. Reproduce the current P88 implementation checks directly

The current theorem frontier is **P88**. Its primary records are:
...
tests/test_heldout_selected_parity_functional_certification.py
