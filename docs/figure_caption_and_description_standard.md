...
- **Restrained arrowheads:** arrowheads are directional cues, not focal objects. For ordinary reader-facing SVGs they should normally occupy about 7 to 10 SVG user units, use `markerUnits="userSpaceOnUse"`, and remain visually smaller than nearby labels, equations, and nodes. Marker geometry must not be multiplied by connector stroke width.
- **Professional line weight:** ordinary connectors should normally remain in the roughly 2 to 3 CSS/SVG-unit range at the native viewBox scale, with rounded caps and joins where appropriate. Thicker connectors require a scientific reason such as encoding a distinct relation or uncertainty, not decoration.
- **Boundary-to-boundary routing:** where a relation connects cards or panels, the visible line should begin at the source boundary and end at the destination boundary whenever geometry permits. Arrowheads should not float in the middle of whitespace or stop conspicuously short of the target.
- **Visual hierarchy:** connectors, shadows, borders, and fills must remain secondary to the mathematical objects and labels. Avoid oversized arrowheads, heavy gradients, excessive shadows, or decorative effects that make a scientific diagram resemble a presentation template or generic generated flowchart.
...
Where geometry is important to interpretation, regression tests should encode the intended layout, for example by checking panel bounds, safe text baselines, connector endpoints, canvas containment, marker units, arrowhead bounds, or required visual stages. The goal is not pixel-perfect styling. The goal is to prevent a future edit or automated regeneration from reintroducing ambiguity, overflow, disconnected arrows, oversized connector furniture, or accidental visual claims.
